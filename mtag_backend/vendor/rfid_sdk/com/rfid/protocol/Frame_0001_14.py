...
class Frame_0001_14(BaseFrame):
    ...
    def __init__(self, *data):
        try:
            ...
            self._CW._CW_MID = 0x14
            self._Data_Len = 0
            self.listData = bytearray()
            if data:
                strParam = data[0].rstrip("-").split("-")
                for item in strParam:
                    self.listData.extend(Helper_String.BytesToArraylist(Helper_String.hexStringToBytes(data[0])))
                self._Data = Helper_String.ArraylisttoBytes(self.listData)
                self._Data_Len = len(self._Data)

        except Exception as e:
            raise RuntimeError("Frame_0001_14(),Error!" + str(e))
